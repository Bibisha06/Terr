...
### ----------------------- LOAD DATA -----------------------
DATA_PATH = 'data/terrain_data.jsonl'  # Ensure this JSONL has lat, lon, elevation, slope, ndvi, etc.
...
for r, c in hiding:
    ...
    if not subset.empty:
        ...
        folium.CircleMarker([lat, lon], radius=5, color="blue", fill=True, fill_opacity=0.5, tooltip="Hiding Spot").add_to(m)
        hiding_count += 1
...
for r, c in choke:
    ...
    if not subset.empty:
        lat, lon = subset.values[0]
        folium.CircleMarker([lat, lon], radius=5, color="orange", fill=True, fill_opacity=0.5, tooltip="Choke Point").add_to(m)
        choke_count += 1
...
for r, c in checkpoints:
    ...
    if not subset.empty:
        lat, lon = subset.values[0]
        folium.CircleMarker([lat, lon], radius=5, color="purple", fill=True, fill_opacity=0.5, tooltip="Surveillance Point").add_to(m)
        checkpoint_count += 1
